...
| 6. Hardening and release | 8 h | 27.7 h | Version 1.3 preparation in progress — TestFlight archive pending |
| **Total** | **40 h** | **50.7 h** | **In progress** |
...
### Version 1.2 release — 2026-09-22

- Status: complete. The owner confirmed version 1.2 was approved and released.
...
- The unsigned Release build, Release Analyze, archive guard, and affected
  47-test suites on both iPhone and iPad Simulators passed before the release.
...
- Status: implementation and local release verification complete.
- Xcode Cloud Build 34 reached App Store validation with marketing version 1.2,
  but Apple rejected it because released version 1.2 is closed to new builds.
- The app target and fail-closed Xcode Cloud guard now use marketing version
  1.3. The automatic Internal TestFlight workflow runs for every `main` push
  and retains its `Jenny Media Internal` post-action.
- The unsigned Release build succeeds and reports `CFBundleShortVersionString`
  1.3, build 1, and bundle identifier `media.jenny.FrameWink`. The archive-mode
  release guard, property-list validation, shell syntax, and diff check pass.
- Remaining gate: merge the exact reviewed change and confirm Xcode Cloud
