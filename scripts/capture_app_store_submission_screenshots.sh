...
if [ -z "$FRAMEWINK_SIMULATOR_ID" ]; then
    FRAMEWINK_SIMULATOR_ID=$(
        DEVELOPER_DIR="$FRAMEWINK_XCODE_DEVELOPER_DIR" xcrun simctl list devices booted \
            | sed -nE '/iPad.*13-inch/{s/.*\(([0-9A-F-]{36})\).*/\1/p;q;}'
    )
fi

if [ -z "$FRAMEWINK_SIMULATOR_ID" ]; then
    echo "No booted 13-inch iPad Simulator was found. Boot one or set FRAMEWINK_SIMULATOR_ID." >&2
    exit 1
fi
...
capture_scenario paywall-features 10-paid-wall-mode-features.jpg
...
if [ "$FRAMEWINK_SCREENSHOT_COUNT" -ne 10 ]; then
    echo "Expected exactly ten submission screenshots, found $FRAMEWINK_SCREENSHOT_COUNT." >&2
    exit 1
fi
...
for FRAMEWINK_SCREENSHOT in "$FRAMEWINK_SCREENSHOT_DIRECTORY"/*.jpg; do
    ...
    case "$FRAMEWINK_SCREENSHOT_METADATA" in
        *"format: jpeg"*"pixelWidth: 2064"*"pixelHeight: 2752"*"hasAlpha: no"*)
            ;;
        *)
            echo "Invalid 13-inch submission screenshot: $FRAMEWINK_SCREENSHOT" >&2
...
echo "Captured ten FrameWink submission screenshots in $FRAMEWINK_SCREENSHOT_DIRECTORY"
